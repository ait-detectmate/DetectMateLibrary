    ...
    def test_segmentation_fields_survive_set_configuration(self):
        ...
        for i in range(10):
            parser_data = schemas.ParserSchema({
                ...
                "parserID": "test_parser",
                "log": "test log",
                "logFormatVariables": {"level": "081109 203615"},
            })
            detector.configure(parser_data)
...
class TestNewValueComboDetectorSegmentationCombos:
    ...
    def _records(segmentation="time"):
        ...
        for i in range(12):
            # var_0 and var_1 both cycle over 3 values -> both STABLE in pass 1,
            # so pass 2 gets a ("var_0", "var_1") combo tracker to look at.
            detector.configure(schemas.ParserSchema({
                "parserType": "test",
                "EventID": 1,
                "template": "Template 1",
                "variables": [f"a{i % 3}", f"b{i % 3}", "constant"],
                "logID": str(i),
                "parsedLogID": str(i),
                "parserID": "test_parser",
                "log": "test log",
                "logFormatVariables": {"ts": str(1700000000 + i * 60)},
            }))
        return detector

    def test_combo_trackers_record_timestamps(self):
        detector = self._records()
        detector.set_configuration(max_combo_size=2)
